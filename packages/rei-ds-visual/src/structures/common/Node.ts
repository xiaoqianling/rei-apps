...
const defaultNodeOptions: Required<
  Omit<NodeOptions, keyof BaseElementOptions | "width" | "height">
> = {
  shape: "circle",
  radius: 20,
  fill: "#eee",
...
export class Node {
  ...
  constructor(options: NodeOptions) {
    ...
    const mergedOptions = {
      ...defaultNodeOptions,
      ...options,
      width: options.width ?? (options.radius ? options.radius * 2 : 40),
      height: options.height ?? (options.radius ? options.radius * 2 : 40),
    }; // 合并默认值
  ...
  public updateStyle(
    style: Partial<
      Pick<
        NodeOptions,
        "fill" | "stroke" | "strokeWidth" | "textColor" | "fontSize"
      >
    >,
  ): void {
    if (style.fill) {
