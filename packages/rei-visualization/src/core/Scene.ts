...
export class Scene extends PIXI.Container {
  ...
  // 插值系数 (0 到 1，越大越快) 用于平湖缩放
  private readonly animationLerpFactor: number = 0.2;
  ...
  private drawGridBackground() {
    const gridSpacing = 100; // 网格线之间的像素距离
  ...
  zoom(scale: number, pointX: number, pointY: number) {
    ...
    // 将缩放比例限制在最小/最大值之间
    newTargetScale = Math.max(
      this.minZoom,
      Math.min(this.maxZoom, newTargetScale),
    );
    ...
    let targetX =
      this.x -
      (afterSimulatedScaleLocalPos.x - beforeScaleLocalPos.x) * newTargetScale;
    let targetY =
      this.y -
      (afterSimulatedScaleLocalPos.y - beforeScaleLocalPos.y) * newTargetScale;
  ...
  /**
   * [DEPRECATED] 旧的直接应用约束的方法。
   * 在 ResizeObserver 中可能仍需要，以立即调整当前状态。
   * 新的交互（平移/缩放）应使用目标状态和动画。
   * @param containerWidth 容器宽度
   * @param containerHeight 容器高度
   */
  applyConstraints(containerWidth?: number, containerHeight?: number): void {
    // 调用新的约束逻辑来计算约束后的状态
    const constrained = this.applyConstraintsToTarget(
      this.x, // 使用当前的 x
      this.y, // 使用当前的 y
      this.scale.x, // 使用当前的 scale
      containerWidth,
      containerHeight,
    );
    // 直接应用约束后的状态到当前场景
    ...
    // 重要：当直接修改当前状态时，也需要同步更新目标状态，
    // 否则动画循环可能会尝试将其拉回旧的目标。
    this.targetX = this.x;
    this.targetY = this.y;
    this.targetScale = this.scale.x;
    this.isAnimating = false; // 强制应用，停止任何进行中的动画
  }
  ...
  fitToContainer(containerWidth: number, containerHeight: number) {
    ...
    const minScale = this.calculateMinScaleToCover(
      containerWidth,
      containerHeight,
    );
   ...
   * 计算场景覆盖容器尺寸所需的最小缩放比例。
   */
  private calculateMinScaleToCover(
    containerWidth?: number,
    containerHeight?: number,
  ): number {
    const cw = containerWidth ?? this.app.screen.width;
  ...
  private updateMinZoom(
    ...
    this.minZoom = this.calculateMinScaleToCover(
      containerWidth,
      containerHeight,
    );
  }
  ...
  private updateAnimation() {
    ...
    // 应用约束到目标值
    const constrainedTarget = this.applyConstraintsToTarget(
      this.targetX,
      this.targetY,
      this.targetScale,
    );
    ...
    const nextScale =
      currentScale +
      (this.targetScale - currentScale) * this.animationLerpFactor;
    const nextX =
      currentX + (this.targetX - currentX) * this.animationLerpFactor;
    const nextY =
      currentY + (this.targetY - currentY) * this.animationLerpFactor;
