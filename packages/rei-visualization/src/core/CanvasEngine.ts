...
export class CanvasEngine {
  ...
  constructor(
    ...
    requestAnimationFrame(() => {
      ...
      if (initialWidth <= 0 || initialHeight <= 0) {
        console.warn(
          `PixiJS 初始化跳过：容器尺寸为零 (${initialWidth}x${initialHeight})。请确保其可见并已布局。`,
        );
        return;
      }
  ...
  private setupInteractions(): void {
    ...
    if (this.isDestroyed || !this.app || !this.app.stage || !this.container) {
      console.warn(
        "setupInteractions 在 PixiJS 应用舞台就绪前或引擎销毁后被调用。",
        {
  ...
  private onDragStart(event: PIXI.FederatedPointerEvent): void {
    ...
    if (
      ...
      this.app.stage &&
      this.scene &&
      (event.target === this.app.stage || event.target === this.scene)
    ) {
  ...
  destroy(): void {
    ...
    if (this.scene && this.app) {
      ...
      console.warn(
        "CanvasEngine.destroy 在初始化完成前或失败后被调用。正在尝试清理...",
      );
