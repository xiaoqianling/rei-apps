...
import { PostPreview, CommunityTag } from '../../types';
...
const PostPreviewCard: React.FC<PostPreviewCardProps> = ({ post }) => {
  const { id, title, author, excerpt, tags, createdAt, likes, commentsCount } = post;
      ...
      <div className={styles.cardFooter}>
        <div className={styles.tagsContainer}>
          {tags.map((tag: CommunityTag) => (
            <Link key={tag.id} to={`/community?tag=${tag.name}`} className={styles.tagLink}>
              #{tag.name}
            </Link>
          ))}
        </div>
