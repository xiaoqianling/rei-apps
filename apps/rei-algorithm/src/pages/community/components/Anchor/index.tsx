...
import type { AnchorItem as ReiAnchorItem } from 'rei-design/anchor'; // Use original type for structure
...
const renderAnchorLinks = (
  ...
  return (
    <ul className={`${styles.anchorList} ${level > 0 ? styles.subList : ''}`}>
      {items.map((item) => (
        <li key={item.id} className={`${styles.anchorItem} ${activeId === item.id ? styles.active : ''}`}>
          <a
            href={`#${item.id}`}
...
const Anchor: React.FC<AnchorProps> = ({ items, offsetTop = 0 }) => {
  ...
  const handleLinkClick = useCallback((id: string, e: React.MouseEvent<HTMLAnchorElement>) => {
